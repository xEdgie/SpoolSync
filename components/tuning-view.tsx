      ...
      <div className="grid gap-4 md:grid-cols-2">
        {TuningGuidesData.data.map((guide : any) => (
            <Card key={guide.name}>
                ...
                <CardContent>
                    <Image src={getGuideImage(guide)} width={16} height={9} alt={guide.name} className="w-full aspect-16/9 object-cover rounded-lg"/>
                    
                    <Link href={`/tuning?guide=${guide.id}`}><Button className="mt-6">Start</Button></Link>
                </CardContent>
            </Card>
        ))}
      </div>
